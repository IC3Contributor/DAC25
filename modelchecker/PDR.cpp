...
void PDR::show_uaiger(){ //suppose all latch init to 0, and the bad property is denoted by the output
    ...
    file << "aag " << (nInputs*depth + nLatches*depth + nAnds*depth) << " " << nInputs*depth << " " << 0 << " 1 " << nAnds*depth+nLatches*depth;
    // if(constraints.size()>0){
    //     file << " " << constraints.size();
    // }
    file << std::endl;
    // 1-d step bmc input
    for(int i = 1; i <= nInputs * depth; ++i) {
        file << 2 * i << std::endl;
    }
    file << dimacs_to_aiger(bad) + 2*nInputs*(depth-1) + 2*(nLatches+nAnds)*(depth-1) << std::endl;
    // 1 step latch
    int falselit = nInputs * depth + 1;
    for(int i = 1; i <= nLatches; ++i) {
        file << 2 * (nInputs * depth + i) << " 2 3" << endl;
    }
    // 1 step and
    for(int i = 0; i < ands.size(); ++i) {
        int o = dimacs_to_aiger(ands[i].o), i1 = dimacs_to_aiger(ands[i].i1), i2 = dimacs_to_aiger(ands[i].i2);
        file << o + 2*nInputs*(depth-1) << " ";
        file << i1 + (i1>=2*(nInputs + 1) ? 2*nInputs*(depth-1) : 0) << " ";
        file << i2 + (i2>=2*(nInputs + 1) ? 2*nInputs*(depth-1) : 0) << std::endl;
    }
    // 2-d step
    for(int d=2; d<=depth; d++){
        ...
        for(int i = 1; i <= nLatches; ++i){
            ...
            if(next >= 2*(nInputs + 1)) //pre: latch or and
                next += 2*nInputs*(depth-1) + 2*(nLatches+nAnds)*(d-2);
            else if(next > 1)//pre: input
                next += 2*nInputs*(d-1);
            else if(next >= 0)
                next = 2*falselit + next;
            file << 2 * (nInputs * depth + (d-1)*(nLatches+nAnds) + i) << " " << next << " " << next << endl;
        }
        // 2-d step and
        for(int i = 0; i < ands.size(); ++i){
            int o = dimacs_to_aiger(ands[i].o), i1 = dimacs_to_aiger(ands[i].i1), i2 = dimacs_to_aiger(ands[i].i2);
            file << o + 2*nInputs*(depth-1) + 2*(nLatches+nAnds)*(d-1) << " ";
            file << i1 + (i1>=2*(nInputs + 1) ? 2*nInputs*(depth-1) + 2*(nLatches+nAnds)*(d-1) : 2*(nInputs)*(d-1)) << " ";
            file << i2 + (i2>=2*(nInputs + 1) ? 2*nInputs*(depth-1) + 2*(nLatches+nAnds)*(d-1) : 2*(nInputs)*(d-1)) << endl;
        }
    }
    file.close(); 
...
void PDR::initialize_heuristic(){
    ...
    //correspond to unprimed_first_dimacs + 0 to unprimed_first_dimacs + nInputs - 1
    for(int i = 2; i <= nInputs+1; i+=10){
        heuristic_lit_cmp->counts[i] = 0.5;
    }
    //correspond to unprimed_first_dimacs + nInputs to unprimed_first_dimacs + + nInputs + nLatches - 1
    for(int i = nInputs+2; i <= nInputs+nLatches+1; i+=10){
        heuristic_lit_cmp->counts[i] = 0.5;
    }
    //correspond to primed_first_dimacs + 0 to primed_first_dimacs + nInputs - 1
